...
const Header = () => {
  ...
  return (
    <header className=''>
      <div className='bg-gradient-to-b from-blue-500 to-blue-400 w-full flex space-x-2 justify-between text-white px-12 '>
        <Link href='/'>
          <a className='flex space-x-2 items-center'>
            ...
            <img src='./images/logo-bg.png' width='150' height='100' />
          </a>
        </Link>
        <button
      ...
      <div className='hidden menu md:hidden'>
        <nav className='bg-blue-400 text-white pb-4'>
          <ul className='flex flex-col space-y-3 justify-between items-center'>
            <li
              className={`w-full py-2 hover:bg-blue-500 ${
                router.pathname == '/' ? 'active-head' : ''
              }`}
            >
              <Link href='/'>
                <a className='block  text-center' onClick={toggleMenu}>
                  Home
                </a>
              </Link>
            </li>
